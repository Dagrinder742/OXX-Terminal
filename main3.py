...
Improved main.py with:
 - safe stream encoding handling
 - better error handling / logging.exception
 - improved buy/sell button detection
 - graceful shutdown for background tasks and clients
 - typing annotations and small refactors
...
# Configure logging early so helpers can log
logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s")


def _ensure_utf8_streams() -> None:
    """Ensure stdout/stderr use UTF-8 where possible (defensive against None encodings)."""
    for stream_name in ("stdout", "stderr"):
        ...
        # only call .lower() if encoding is a str
        if isinstance(enc, str) and enc.lower() != "utf-8":
            try:
                # reconfigure may not be available for all streams or Python versions
                stream.reconfigure(encoding="utf-8")
                logging.debug("Reconfigured %s to utf-8", stream_name)
            except (AttributeError, ValueError, OSError) as e:
                # log the specific error, don't swallow silently
                logging.debug("Could not reconfigure %s: %s", stream_name, e)


_ensure_utf8_streams()

# Imports that depend on the terminal / textual environment and the project's modules
...
class AuthModal(ModalScreen):
    ...
    def compose(self) -> ComposeResult:
        with Vertical(id="dialog"):
            ...
            yield Static("Enter your API credentials. Press Enter to submit.", id="auth-instructions")
    ...
    def _submit_credentials(self) -> None:
        ...
        if not (api_key and secret_key and passphrase):
            try:
                self.query_one("#auth-msg", Static).update("[bold red]All fields are required! Please fill out all inputs.[/bold red]")
            except Exception:
                logger.warning("Could not update auth modal message widget.")
            return
        ...
        try:
            ...
            logger.exception("Failed to save credentials to encrypted vault")
            try:
                self.query_one("#auth-msg", Static).update("[bold red]Failed to save credentials. Check logs.[/bold red]")
            except Exception:
                logging.exception("Could not update auth modal message widget")


class OKXTerminalApp(App):
    """A fully asynchronous, real-time OKX TUI trading terminal with live market depth grids."""

    current_price: reactive = reactive("Connecting...")
    high_24h: reactive = reactive("---")
    low_24h: reactive = reactive("---")
    volume_24h: reactive = reactive("---")

    def __init__(self) -> None:
        super().__init__()
        self.instrument_id: str = "BTC-USD"
        self.cached_asks: List[Any] = []
        self.cached_bids: List[Any] = []
        self.cached_trades: List[Any] = []
        self.bg_worker: Optional[asyncio.Task] = None
        self.bot_worker: Optional[asyncio.Task] = None
        self.client: Optional[OKXPublicClient] = None
        self.current_timeframe: str = "15m"
        self.strategy_manager: StrategyManager = StrategyManager()
        self.session_fills: List[Dict[str, Any]] = []
    ...
    /* ... (omitted for brevity; unchanged CSS from original) ... */
    """
    ...
    async def on_mount(self) -> None:
        try:
            ...
            logger.exception("Error loading credentials from EncryptedVault")
            creds = {}

        api_key = creds.get("api_key")
        if not api_key:
            # push_screen callback receives a bool result; keep behavior
    ...
    def on_button_pressed(self, event: Button.Pressed) -> None:
        ...
        # Determine side more robustly using classes
        try:
            classes = set(event.button.classes or ())
        except Exception:
            classes = set()

        if "buy-btn" in classes:
            side = "buy"
        elif "sell-btn" in classes:
            side = "sell"
        else:
            # fallback: attempt to infer from label
            lbl = getattr(event.button, "label", None)
            text = getattr(lbl, "text", "") if lbl is not None else ""
            side = "buy" if text.upper().startswith("BUY") else "sell"
        ...
        try:
            ...
            self.notify("Internal error reading order inputs.", severity="error", title="Order Error")
            logger.exception("Failed to read order inputs")
            return
    ...
    def action_switch_instrument(self, new_inst: str) -> None:
        ...
        try:
            ...
            logger.debug("Could not update header-bar widget on instrument switch.")
        ...
        try:
            ...
            logger.debug("Could not clear trades widget on switch")
        ...
        try:
            ...
            logger.exception("Failed to start public client for new instrument")
    ...
    def action_start_bot(self, strategy_type: str = "GRID") -> None:
        try:
            ...
            self.notify("Unable to read current price; cannot start bot.", severity="error")
            logger.exception("Invalid current price when starting bot")
            return
        ...
        try:
            ...
            self.notify("Invalid investment/amount value.", severity="error")
            return
        ...
        try:
            ...
            logger.exception("Failed to start strategy bot")
            self.notify("Failed to start bot; check logs.", severity="error")
    ...
    def action_stop_bot(self) -> None:
        try:
            ...
            self.notify("Error stopping bots; see logs.", severity="error")

    def update_bot_ui(self) -> None:
        summary = self.strategy_manager.get_status_summary()
        status_color = "green" if summary.get("status") == "ACTIVE" else "red"
        ...
        try:
            self.query_one("#bot-status", Static).update(f"Engine Status: [bold {status_color}]{summary.get('status')}[/bold {status_color}]")
            self.query_one("#bot-metrics", Static).update(
                f"Active Bots: {summary.get('count', 0)} | Session PnL: ${live_pnl:,.2f}\n[dim]{summary.get('details', '')}[/dim]"
            )
        except Exception:
            logger.debug("Could not update bot UI widgets")
    ...
    async def _run_bot_execution_loop(self, bot_id: str) -> None:
        ...
        while bot_id in self.strategy_manager.active_bots:
            try:
                ...
                # Task was cancelled (stop requested) — exit cleanly
                logger.debug("Bot execution loop cancelled")
                break
            except Exception:
                logger.exception("Error in bot execution loop")
    ...
    def refresh_chart(self) -> None:
        async def load_task() -> None:
            try:
                ...
                # Use from_ansi to preserve coloring/terminal sequences
                try:
                    ...
                    logger.debug("Chart widgets not available for update")
            except Exception:
                logger.exception("Could not update candlestick chart widget")

        self.run_worker(load_task)

    async def _execute_order_task(self, side: str, ord_type: str, size: str, price: str, tp: str, sl: str, tag: str = "Manual", bot_id: Optional[str] = None) -> None:
        from okx_private import OKXPrivateClient  # local import to avoid import-time dependency
        ...
        try:
            ...
            logger.exception("Error calling private place_order API")
            self.notify("Order submission failed: API error", severity="error", title="API Error")
            return
        ...
        if code == "0":
            ...
            try:
                ...
                logger.exception("Failed to update session fills")

            if bot_id:
                try:
                    self.strategy_manager.update_bot_fill(bot_id, side, float(exec_px), float(size))
                except Exception:
                    logger.exception("Failed to update bot fill")
            ...
            logger.warning("Order failed: %s", msg)
    ...
    def update_history_display(self) -> None:
        ...
        for f in self.session_fills:
            color = "green" if f.get("side") == "BUY" else "red"
            tag_color = "#3399ff" if f.get("tag") == "Manual" else "#ffcc00"
            lines.append(
                f"[{tag_color}]{f.get('tag')}[/{tag_color}] | {f.get('time')} | "
                f"[{color}]{f.get('side')}[/{color}] {f.get('sz')} @ {f.get('px')}"
            )
        ...
        try:
            ...
            logger.debug("History display update skipped")

    def _start_terminal_services(self) -> None:
        # Slightly less frequent header updates to reduce CPU usage
        ...
        # Charts are somewhat expensive; 30s is OK but could be tuned per usage
        self.set_interval(30.0, self.refresh_chart)
        try:
            self.client = OKXPublicClient(instrument_id="BTC-USD", callback=self.handle_ws_data)
            self.bg_worker = asyncio.create_task(self.client.connect_market_streams())
        except Exception:
            logger.exception("Failed to initialize public client")
    ...
    async def update_portfolio_balance(self) -> None:
        ...
        if result.get("code") == "0":
            ...
            try:
                ...
                logger.debug("Could not update portfolio balance widget")
        else:
            logger.debug("Account balance API returned non-zero code")
    ...
    async def update_open_orders_and_positions(self) -> None:
        ...
        try:
            ...
            logger.exception("Failed to fetch orders/positions")
            return
        ...
        if pos_res.get("code") == "0":
            ...
            active_pos = [p for p in positions if float(p.get("pos", 0) or 0) != 0]
            if active_pos:
        ...
        try:
            ...
            logger.debug("Could not update positions/orders widget")

    def log_action(self, message: str) -> None:
        """Prepend a short log message to the execution log widget; fall back to logger if not available."""
        try:
            log_widget = self.query_one("#execution-log-content", Static)
            current_text = getattr(log_widget, "renderable", "")
            ...
            if channel == "tickers":
                ...
                for book in data:
                    ...
                    if action == "snapshot":
                        ...
                        for ask in raw_asks:
                            try:
                                ...
                                logging.debug("Skipping malformed ask entry: %s (%s)", ask, e)
                                continue
                        ...
                        for bid in raw_bids:
                            ...
                            try:
                                ...
                                logging.warning(f"skipping malformes bid entry", {e})
                    ...
                    try:
                        ...
                        logger.debug("Order book update skipped during shutdown or widget missing")
                ...
                for trade in data:
                    try:
                        ...
                        logging.debug("Skipping malformed trade entry: %s (%s)", trade, e)
                        continue
                ...
                try:
                    ...
                    logger.debug("Trade feed update skipped during shutdown")
        except Exception:
            logger.exception("Unexpected error handling websocket data")
    ...
    def update_header_display(self) -> None:
        try:
            ...
            logger.debug("Header update skipped (widget missing or app shutting down)")

    async def on_unmount(self) -> None:
        """Attempt to gracefully cancel background workers and close client connections."""
        try:
            if self.bg_worker and not self.bg_worker.done():
                self.bg_worker.cancel()
                await asyncio.sleep(0)  # give the loop a chance to cancel
        except Exception:
            logger.exception("Error cancelling bg_worker")

        try:
            if self.bot_worker and not self.bot_worker.done():
                self.bot_worker.cancel()
                await asyncio.sleep(0)
        except Exception:
            logger.exception("Error cancelling bot_worker")

        try:
            if self.client:
                # Try a safe disconnect if the client supports it
                disconnect = getattr(self.client, "disconnect", None)
                if callable(disconnect):
                    await asyncio.to_thread(self.client.disconnect)
        except Exception:
            logger.exception("Error disconnecting public client")
