...
class OKXTerminalApp(App):
    ...
    def on_resize(self, event) -> None:
        """Handles liquid layout state toggles with aggressive debouncing for mobile."""
        # Cancel any pending refresh task
        if self.resize_timer:
            self.resize_timer.stop()
        
        # Schedule a new refresh once resizing has 'settled' (500ms for mobile)
        self.resize_timer = self.set_timer(0.5, self.execute_liquid_refresh)
    ...
    def execute_liquid_refresh(self) -> None:
        ...
        try:
            ...
            # More aggressive adaptive triggers for mobile
            if width > 190:
                hub_container.styles.layout = "horizontal"
                hub_c.styles.display = "block"
            elif width > 130: # Switched to vertical sooner than desktop
                hub_container.styles.layout = "horizontal"
        ...
        async def load_task():
            try:
                ...
                data = await asyncio.to_thread(
                    ...
                    limit=50 # Reduced from 80 for high-efficiency mobile rendering
                )
